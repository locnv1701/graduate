...
Coloring.prototype.result = function (resultCallback) {
    console.log("buoc 7: Coloring.prototype.result")
    this.calculate(true);
...
Coloring.prototype.calculate = function (fillUpText = false) {
    ...
    // xáo trộn thứ tự đinh của đồ thị
    this.addSimpleAndRandomOrders(listOfOrders);

    // tạo ra các danh sách sắp xếp các đỉnh dựa trên bậc của đỉnh và thêm vào listOfOrders
...
/*
    Việc xáo trộn thứ tự của các đỉnh trong đồ thị (sử dụng hàm shuffleArray) 
    có thể giúp ích cho thuật toán tô màu đồ thị (graph coloring) theo các cách sau:

    1. Diversify Starting Point: 
        Bằng cách xáo trộn thứ tự đỉnh, thuật toán tô màu có thể bắt đầu từ một đỉnh ngẫu nhiên, 
        thay vì luôn bắt đầu từ đỉnh đầu tiên trong danh sách. Điều này có thể dẫn đến các kết 
        quả tô màu khác nhau dựa trên thứ tự khởi đầu khác nhau, giúp tìm ra một tô màu tối ưu hơn.
    2. Minimize Color Conflicts: 
        Xáo trộn thứ tự đỉnh có thể làm giảm xác suất xảy ra xung đột màu (color conflict) giữa các
        đỉnh gần nhau trong đồ thị. Nếu các đỉnh có liên quan đến nhau trong thứ tự gần nhau, có khả
        năng chúng sẽ được tô cùng một màu (vì chúng gần nhau trong danh sách). Bằng cách xáo trộn, 
        chúng được phân tán rộng hơn, giúp giảm xung đột màu và tạo ra tô màu tối ưu hơn.
    3. Avoid Local Minima: 
        Trong quá trình tô màu, thuật toán có thể bị mắc kẹt ở một vị trí cục bộ (local minimum), 
        nghĩa là không thể tô màu thêm đỉnh nào mà không làm tăng số màu sử dụng. Bằng cách xáo trộn
        thứ tự đỉnh, có thể tạo ra cơ hội để thoát khỏi vị trí cục bộ này và tìm ra một giải
        pháp tối ưu hơn.

    Tóm lại, việc xáo trộn thứ tự đỉnh là một chiến thuật tốt để cải thiện hiệu suất và chất lượng 
    của thuật toán tô màu đồ thị, đặc biệt là khi các đỉnh có mối quan hệ gần nhau trong danh sách 
    ban đầu có thể dẫn đến kết quả không tối ưu.
*/
...
Coloring.prototype.addBasedOnDegree = function (listOfOrders, connectedVertex) {
    ...
    // tạo mảng vertexDegree lưu index là đỉnh, degree là cấp của đỉnh đó
    for (var i = 0; i < this.graph.vertices.length; i++) {
        var degree = 0;
    ...
    // sắp xếp vertexDegree theo thứ tự giảm dần của cấp của đỉnh
    vertexDegree.sort(
        function (a, b) {
    ...
    // một danh sách vertexOrder mới được tạo ra dựa trên thứ tự của các đỉnh sau khi đã được sắp xếp theo cấp
    var vertexOrder = [];
    ...
    // thêm danh sách vừa tạo vào listOfOrders
    listOfOrders.push(vertexOrder);
