...
function swapControlDiv() {
	...
	if (swapped) {
		...
		reorderSibling(document.getElementById('generalAnimationControlSection'), document.getElementById('canvas-tree'));
...
function AnimationManager(objectManager, dom) {
	...
	this.startNextBlock = function () {
		console.log("bước 10: this.startNextBlock", this.animatedObjects)
		...
		while (this.currentAnimation < this.AnimationSteps.length && !foundBreak) {
			...
			// VD: this.cmd("CreateCircle", treeNodeID, insertedValue, this.startingX, AVL.STARTING_Y);
			// tạo một node mới
			if (nextCommand[0].toUpperCase() == "CREATECIRCLE") {
				this.animatedObjects.addCircleObject(parseInt(nextCommand[1]), nextCommand[2]);	// tạo node mới vào thêm vào objectManager.Nodes[treeNodeID]
				if (nextCommand.length > 4) {
					this.animatedObjects.setNodePosition(parseInt(nextCommand[1]), parseInt(nextCommand[3]), parseInt(nextCommand[4]));	// xét vị trí cho node
				}
				undoBlock.push(new UndoCreate(parseInt(nextCommand[1])));

			}

			// VD: this.cmd("Connect", tree.graphicID, elem.graphicID, AVL.LINK_COLOR);
			// tạo cạnh nối giữa 2 node chỉ định
			...
			// VD: this.cmd("Move", tree.graphicID, tree.x, tree.y);
			// move này chỉ là thêm đối tượng và điểm đích của nó vào currentBlock( mảng lưu các đối tượng cần move)
			else if (nextCommand[0].toUpperCase() == "MOVE") {
				var objectID = parseInt(nextCommand[1]);

				console.log("objectID", objectID);

				// *currentBlock lưu danh sách các đối tượng cần tạo hoạt ảnh di chuyển
				// khởi tạo SingleAnimation(id, fromX, fromY, toX, toY), lưu vị trí đầu và cuối của một object cần dịch chuyển
				// sau đó được push vào currentBlock, khi animationManager.update(); được gọi currentBlock sẽ được duyệt qua
				// các object được currentBlock lưu id, sẽ được cập nhật liên tục vị trí với và vẽ lại, tao hoạt ảnh di chuyển 
			...
			// VD: this.cmd("SetForegroundColor", treeNodeID, AVL.FOREGROUND_COLOR);
			// xét màu viền cho node chỉ định
			...
			// VD: this.cmd("SetBackgroundColor", treeNodeID, AVL.BACKGROUND_COLOR);
			// xét màu nền cho node chỉ định
			...
			// VD: this.cmd("Disconnect", A.parent.graphicID, A.graphicID, AVL.LINK_COLOR);
			// xóa connect giữa 2 node
			...
			// VD: this.cmd("SetText", 0, "Found null tree, inserting element");
			// set text cho một đối tượng node chỉ định theo id
			else if (nextCommand[0].toUpperCase() == "SETTEXT") {
				if (parseInt(nextCommand[1]) == 0) {
					this.document.getElementById('message').innerHTML = nextCommand[2] !== " " ? nextCommand[2] : "DONE";
				} else {
			...
			// VD: this.cmd("Delete", this.highlightID);
			// xóa bỏ object trong Nodes ví dụ HighlightCircle sau khi thực hiện xong việc vẽ highlight sẽ bị xóa bỏ
			...
			// VD: this.cmd("CreateHighlightCircle", this.highlightID, AVL.HIGHLIGHT_COLOR, tree.right.x, tree.right.y);
			// thêm HighlightCircle vào Nodes tại index Nodes[this.highlightID]
				...
				if (nextCommand.length > 5) {
					...
					this.animatedObjects.addHighlightCircleObject(parseInt(nextCommand[1]), this.parseColor(nextCommand[2]), 20);	// 20 là bán kính
				}
				if (nextCommand.length > 4) {
					// set vị trí cho Nodes[this.highlightID] (là HighlightCircle vừa tạo)
			...
			// VD: this.cmd("CreateLabel", labelID, 1, this.startingX - 20, AVL.STARTING_Y - 20);
			// thêm label cho node chỉ định 
			...
			// VD: this.cmd("SetEdgeHighlight", A.graphicID, B.graphicID, 1);
			// set highlight cho cạnh
	...
	this.StartNewAnimation = function (commands) {
		console.log("bước 9: this.StartNewAnimation", commands);
		...
		this.fireEvent("AnimationStarted", "NoData");	// khi đang vẽ hoạt ảnh thì gọi AnimationStarted để vô hiệu hóa các button 
		timer = setTimeout('timeout()', 30);
	...
	// cập nhật trạng thái và vị trí của các đối tượng trước khi vẽ lại chúng, tạo hoạt ảnh di chuyển
	this.update = function () {
		console.log("animationManager.update")
		if (this.currentlyAnimating)		// kiểm tra có hoạt ảnh nào đang được thực hiện hay không??? -> nếu đang có hoạt ảnh diễn ra tiếp tục xử lý
		{
			this.currFrame = this.currFrame + 1; // tăng curr frame, đại diện cho số khung hình hiện tại của hoạt ảnh
			var i;
			console.log("this.currentBlock", this.currentBlock)
			for (i = 0; i < this.currentBlock.length; i++)	// duyệt qua các đối tượng đang tham gia trong khối hoạt ảnh hiện tại
			{
				if (this.currFrame == this.animationBlockLength || (this.currFrame == 1 && this.animationBlockLength == 0)) {
					// Kiểm tra this.currFrame để xác định liệu hoạt ảnh đã hoàn thành (đạt đủ số khung hình) chưa. 
					// Nếu đã hoàn thành hoặc mới bắt đầu và animationBlockLength là 0, thì đặt vị trí của đối tượng vào vị trí đích (toX, toY).

					this.animatedObjects.setNodePosition(this.currentBlock[i].objectID,
						this.currentBlock[i].toX,
						this.currentBlock[i].toY);
				}
				else if (this.currFrame < this.animationBlockLength)	// Nếu this.currFrame chưa đủ animationBlockLength
				{
					var objectID = this.currentBlock[i].objectID;
					...
					// tính toán vị trí của đối tượng tại mỗi khung hình dựa trên việc lerp (tích hợp tuyến tính) giữa vị trí hiện tại và vị trí đích. 
					// Điều này tạo ra hiệu ứng dịch chuyển đối tượng một cách mượt mà từ vị trí hiện tại đến vị trí đích.
				}
			}


			// Kiểm tra xem hoạt ảnh đã hoàn thành chưa. Nếu đã hoàn thành, kiểm tra xem có đang thực hiện "undo" (hoàn tác) hoạt ảnh hay không. 
			// Nếu đang hoàn tác, kết thúc khối hoạt ảnh hiện tại và kiểm tra xem còn hoạt ảnh nào trong ngăn xếp hoàn tác không. Nếu còn, tiếp tục với hoạt ảnh tiếp theo.
			if (this.currFrame >= this.animationBlockLength) {
				...
				else //	Nếu không phải hoàn tác và hoạt ảnh đang tạm dừng hoặc hoạt ảnh hiện tại chưa hoàn thành, chờ sự kiện và đánh dấu rằng đang chờ đợi sự kiện "AnimationWaiting."
				{
			...
			this.animatedObjects.update();
			// Cuối cùng, gọi this.animatedObjects.update(); để cập nhật trạng thái của tất cả các đối tượng hoạt hình, 
			// giúp vẽ lại màn hình với trạng thái mới sau mỗi khung hình.
